...
const PostsFeed = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      console.log(
        '📋 PostsFeed: Loaded admin job vacancies from adminFeedPosts sessionStorage:',
        storedPosts.length
      )

      // Filter only job vacancies from admin feed
      const jobVacancies = storedPosts.filter(
        (post) => post.type === 'job_vacancy' && post.source === 'adminFeed'
      )
      setPosts(jobVacancies)
    } catch (error) {
  ...
  const handleApplySuccess = (application) => {
    ...
    try {
      ...
      sessionStorage.setItem('feedApplications', JSON.stringify(updatedFeedApplications))
      ...
      const updatedDashboardApplications = [application, ...dashboardApplications]
      sessionStorage.setItem('dashboardApplications', JSON.stringify(updatedDashboardApplications))
      ...
      console.log('✅ PostsFeed: Application submitted and mirrored to Dashboard')
    } catch (error) {
  ...
  const filteredAndSortedPosts = posts
    .filter(post => {
      ...
      const matchesFilter = selectedFilter === 'all' || 
        (selectedFilter === 'recent' && new Date(post.createdAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))
    ...
    <div className="min-h-screen bg-gray-50 pt-20">
      ...
      <DashboardJobApplyModal
        ...
        prefillData={
          selectedPost
            ? {
                jobRole: selectedPost.title,
                coverLetter: `I'm interested in the ${selectedPost.title} position at ${selectedPost.company}.`,
              }
            : {}
        }
      />
