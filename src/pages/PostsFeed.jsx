...
const PostsFeed = () => {
  ...
  const filteredJobs = useMemo(() => {
    ...
    if (import.meta.env.DEV) {
      console.log('📋 PostsFeed: Filtered admin jobs:', filtered.length)
      console.log('📋 PostsFeed: Original jobs:', jobsData.length)
      console.log(
        '📋 PostsFeed: Filtered out (non-admin):',
        jobsData.length - filtered.length
      )
    }
    ...
    const loadJobs = async () => {
      try {
        ...
        if (import.meta.env.DEV) {
          ...
          // Log only admin jobs for debugging
          const adminJobs = fetchedJobs.filter(
            (job) => job.postedBy === 'admin'
          )
          console.log('📋 PostsFeed: Admin jobs found:', adminJobs.length)
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (jobs.length > 0 && import.meta.env.DEV) {
